...
export class SettingService extends BaseService {
  protected url: string = 'api/services/app/Setting';

  getStartByKey(key: string){
    return this.get(`${this.url}/GetStartByKey?Key=${key}`, key);
  }

  getByKey(key: string): Observable<any> {
    return this.get(`${this.url}/GetByKey?Key=${key}`, key);
  }
